# MODEL.md - AI Assistant Guide for log-simulator
...
- **Git Branch Strategy**: Feature branches prefixed with `feat/` for new features
...
1. **Create Feature Branch**
   - Branch naming: `feat/descriptive-name` (e.g., `feat/add-workspace-logs`)
   - Always create from `main` branch
   - Command: `git checkout -b feat/feature-name`

2. **Create Task Checklist**
   - Create `task.md` artifact in the brain directory
   - Break down work into granular, trackable items
...
4. **Testing & Verification**
   ...
   - **UI Testing**: If changes affect the Web UI, run local server and verify:
     - Start server: `PYTHONPATH=src uvicorn log_simulator.api.main:app --port 8000`
     - Test UI functionality manually or with browser automation
     - Verify dropdown updates, schema loading, scenario selection, etc.
   - Update `task.md` to mark verification steps complete
...
6. **Create Pull Request**
   - Use GitHub PR URL with pre-filled title and body parameters
   - Follow existing PR format (see previous PRs for template)
   - Include: Summary, Changes, Testing, Impact sections
...
1. **Plan First**: Create implementation plan and task checklist
2. **Structure Check**:
...
4. **Testing**:
   ...
   - Test UI changes locally if applicable
